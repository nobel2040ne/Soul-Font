...
from skimage.metrics import structural_similarity as ssim  # ✅ 수정된 코드
...
class UNet(object):
    ...
    def restore_model(self, saver, model_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            print("Restoring parameters from {}".format(ckpt.model_checkpoint_path))
            saver.restore(self.sess, ckpt.model_checkpoint_path)
            step = int(ckpt.model_checkpoint_path.split('-')[-1])  # 예: unet.model-1500 → 1500
            return step
        else:
            raise Exception("No checkpoint found at {}".format(model_dir))
    ...
    def validate_model(self, val_iter, epoch, step):
        ...
        merged_pair = np.clip(merged_pair * 255.0, 0, 255).astype(np.uint8)
        merged_pair = np.squeeze(merged_pair)
...
#                print("[%d] SSIM: %.5f" % (sample_num, ssim_i))
    ...
    def train(self, lr=0.0002, epoch=100, schedule=10, resume=True, flip_labels=False,
        ...
        if resume:
            ...
            counter = self.restore_model(saver, model_dir)   # 수정됨: 복원 먼저
            print("✅ Restored from checkpoint")    # 수정됨: 복원 확인 로그
        else:
            tf.global_variables_initializer().run() # 수정됨: 초기화는 복원 실패 시만
            counter = 0
            print("✅ Initialized from scratch")    # 수정됨: 초기화 확인 로그
