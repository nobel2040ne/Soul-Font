...
def get_offset(ch, font, canvas_size):
    ...
    font_bbox = font.getbbox(ch)

    font_width = font_bbox[2] - font_bbox[0]
    font_height = font_bbox[3] - font_bbox[1]

    offset_x = (canvas_size - font_width) / 2 - font_bbox[0]
    offset_y = (canvas_size - font_height) / 2 - font_bbox[1]
...
def get_font_offset(charset, font, canvas_size, filter_hashes):
    ...
    # if count>0:
    #     font_offset /= count
    # return font_offset(int)
    _charset = charset[:]
    np.random.shuffle(_charset)
    sample = _charset[:2000]
    font_offset = np.array([0.0, 0.0])  # 초기값을 float64로 설정
    count = 0
    for c in sample:
        font_img = draw_single_char(c, font, canvas_size, 0, 0)
        font_hash = hash(font_img.tobytes())
        if font_hash not in filter_hashes:
            font_offset += np.array(get_offset(c, font, canvas_size), dtype=np.float64)  # 반환값을 float64로 변환
            count += 1
    if count > 0:
        font_offset /= count
    return font_offset  # 여기에서 올바르게 numpy array를 반환
...
def draw_handwriting(ch, src_font, canvas_size, src_offset, dst_folder):
    s = ch.encode('raw_unicode_escape').decode('utf-8').replace("\\u", "").upper()  # 바이트로 변환 후, str로 다시 디코딩
    dst_path = os.path.join(dst_folder, "uni" + s + ".png") 
    ...
    if handwriting_dir:
        ...
        for c in charset:
            ...
            if e:
                ...
                e.save(os.path.join(sample_dir, "%d_%s_train.png" % (label, c.encode('raw_unicode_escape').decode('utf-8').replace("\\u","").upper())))
                train_set.append(c)
                count += 1
                if count % 100 == 0:
        ...
        for c in charset:
            ...
            if e:
                ...
                e.save(os.path.join(sample_dir, "%d_%s_val.png" % (label, c.encode('raw_unicode_escape').decode('utf-8').replace("\\u","").upper())))
                count += 1
                if count % 100 == 0:
