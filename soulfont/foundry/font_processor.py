...
from foundry.inference import main as inference_main
from foundry.char_layout import (
...
LANGUAGE = 'kor'
# The Django project root: two levels up lands outside foundry/, where data/,
# checkpoints/ and static/ live.
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# Everything the pipeline writes per upload lives under one parent so it is obvious at a
# glance what is generated and what is source. Was three separate top-level directories
# (FONT/, style/, configs/) plus static/outputs/, which put working data inside the
...
def script_fit_scales(trace_dir, sample=200):
    ...
    """
    from foundry.refine_metrics import (KOR_HEIGHT_PERCENTILE, KOR_TARGET_HEIGHT, LATIN_CAP_TARGET)
...
def _probe_font(paths, work_dir, name):
    ...
    from foundry.refine_metrics import refine_metrics
...
class FontStyleProcessor:
    def __init__(self, pdf_path, charset_path=DEFAULT_CHARSET, device_name='auto', use_amp=False):
        ...
        self.output_dir = os.path.join(WORKDIR, "crops", self.base_name)
        ...
        self.yaml_path = os.path.join(WORKDIR, "configs", f"{self.base_name}.yaml")
        self.checkpoint = os.path.join(PROJECT_ROOT, "checkpoints", "korean-handwriting.pth")
        self.save_dir = os.path.join(WORKDIR, "glyphs", self.base_name)
    ...
    def trim_and_save_images(self):
        ...
        subprocess.run([
            sys.executable, os.path.join(PROJECT_ROOT, "foundry", "crop.py"),
