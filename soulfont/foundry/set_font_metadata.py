...
def ascii_postscript_name(font_name, user_id=''):
...
# The only four styles name IDs 1/2 can express. Anything else has to be named
# through IDs 16/17 instead — see _legacy_names.
RIBBI_STYLES = frozenset({'Regular', 'Bold', 'Italic', 'Bold Italic'})


def _legacy_names(font_name, style):
    """Family/subfamily for name IDs 1/2, which only understand the four RIBBI styles.

    A weight outside that set (Light here; Thin, Medium, Black if they are ever added)
    cannot say so in ID 2, so it folds the weight into the family name and claims
    "Regular" — IDs 16/17 then carry the real family and weight. Avenir Next, Pretendard
    and every other multi-weight family do exactly this. Naming ID 2 "Light" instead
    looks tidier but is not a value the field can hold: apps that read only IDs 1/2
    (Word's classic path among them) then list Light as a separate family rather than
    offering it as a weight of one.
    """
    if style in RIBBI_STYLES:
        return font_name, style
    return f'{font_name} {style}', 'Regular'


def _apply_weight(font, weight):
    """Set OS/2 usWeightClass + the bold style bits so the OS groups & styles the weight.

    The typographic family (ID 16) is identical across weights, so Light, Regular and
    Bold install as one family; ID 1 differs for non-RIBBI weights (see _legacy_names).
    """
...
def apply_metadata(ttf_path, font_name, out_path=None, user_id='',
    ...
    `weight` is 'Light', 'Regular', or 'Bold'; all three share one typographic family
    so they group under a single name in font menus.
    """
    out_path = out_path or ttf_path
    style = WEIGHTS.get(weight, WEIGHTS['Regular'])[1]
    font = TTFont(ttf_path)
    name = font['name']
    ps_name = ascii_postscript_name(font_name, user_id)
    # Clears the generator's leftover records first, so user-supplied ones below stick.
    _apply_font_defaults(font)

    legacy_family, legacy_style = _legacy_names(font_name, style)
    _set_name(name, 1, legacy_family)             # Font Family (RIBBI-safe)
    _set_name(name, 2, legacy_style)              # Font Subfamily (RIBBI-safe)
